...
value_t InterpreterScope::getValueOf(std::string identifier) {
	/*std::cout << "in get value of the value of identifier is]=" << identifier << std::endl;
	std::cout << "in get value of the value of identifier is]=" << variableSymbolTable[identifier].second.i << std::endl;*/
	////checkPurpose
	//check("printing eht content of variable symbol table");
	//for (auto i : variableSymbolTable) {
	//	std::cout << i.first<<"\n\n\n";
	//	std::cout << i.second.second.i<<"\n\n\n";
	//}
...
//	std::cout << "\n in the int declaratino= " << identifier << "value =" << n << std::endl;
	value_t v;
	v.i = n;
	variableSymbolTable[identifier] = std::make_pair(parser::TYPE::INT, v);
	//std::cout << "after declaration ="<<variableSymbolTable[identifier].second.i << std::endl;
...
void Interpreter::visit(parser::ASTProgramNode* prog) {
	for (auto statement : prog->_statements) {
	 ...
	 void Interpreter::visit(parser::ASTAssignmentNode* assignment) {
		 ...
		 {
			 /*std::cout << count << " scope back" << std::endl;
			 count++;*/
		 };
		 ...
		 case parser::TYPE::INT: {
			/* check("assignment");
			 std ::cout << "current expression value =" << _currentExpressionValue.i;*/
		 ...
		 //std::cout << _scopes.back()->getValueOf(i);
	...
	void Interpreter::visit(parser::ASTLiteralNode<std::string>* lit) {
	 ...
	 void Interpreter::visit(parser::ASTIdentifierNode* identifier) {
		...
		// std::cout << "int the  in=dentifier node value of current expression value =" 
			// << _currentExpressionValue.i;
		 //check("identifier node interpreted sucessfully");
		/* check("type =" + static_cast<int>(_currentExpressionType));
		 check("value =");*/
		 /*std::cout<< _currentExpressionValue. <<std::endl*/
	 ...
	 void Interpreter::visit(parser::ASTBinaryExprNode* binaryStatement) {
		 ...
		 if (op == "+" or op == "-" or op == "/" or op == "*") {
			 ...
			 if (firstExpType == parser::TYPE::INT && secExpType == parser::TYPE::INT) {
				 ...
				 {
					/* check("\n\n\nplus operator.....");
					 std::cout << "sec exp value" << secExpVal.i<<std::endl;
					 std::cout << "first exp value" << firstExpVal.i<<std::endl;*/
					 
					 v.i = firstExpVal.i + secExpVal.i;
					 //std::cout << "whole exp value" << v.i << std::endl;
				 }
